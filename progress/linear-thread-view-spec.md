...
- **逐轮渲染**（每个 thread 节点一块，纵向堆叠、整体可滚动）：
  ...
  - 工具调用：`<ToolCallsPanel toolCalls={node.toolCalls} />`（直接复用）。**S83 起改为 `<ToolTimeline>`（`components/tools/`），`ToolCallsPanel` 已删。**
...
- 复用既有：`MD_COMPONENTS`(`@/lib/md-components`)、`ToolCallsPanel`（S83 起是 `ToolTimeline`）、`ancestorsOf`(`@/lib/collapsed`)、`layoutNodes`(`@/lib/layout`)、`CliResumeButton`/`CopyButton`。不重造 markdown/工具渲染。

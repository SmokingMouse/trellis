...
- **ttyd 不随父进程死，必须主动收尸**（S77 实测）：kill 掉 trellis 后 spawn 出的 ttyd 仍在原端口 LISTEN 变孤儿，下次启动 `pickPort` 跳过被占端口漂到下一个 → **每重启泄漏一个进程 + 一个端口**。`lib/server/ttyd.ts` 的 `reapOrphans()` 按 `pgrep -f "ttyd .*titleFixed trellis"` 签名清理。**tmux session 刻意不清**——它们活着正是「重启不丢终端」这个特性本身（实测：收尸重启后 `tmux capture-pane` 里仍留着上一轮的命令与输出）。

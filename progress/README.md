...
## Current Focus
两台实例待重部（S85 修复未生效）；devbox 部署通路刚跨平台修好（S86），先在那儿跑 install-service + deploy。S1 与 worktree 建完即用（S87）待验收。

## Goals

- [ ] **S1: Project/Workspace 层级 + 工作区终端** → [spec](project-workspace-layer.md) · [ADR](decisions/2026-07-27-project-workspace-layer.md)
  P0/P1/P2 均已落地。判据是**行为指标**：一周内 worktree 里的 session 数 > 0 —— 不是功能做完。
  **首轮实测为 0**；S83、S87 各修掉一批可用性堵点后重新计时。S2/S3/S4 依赖它，见 ADR。
...
- `sessions.md` — 最近 5 条 session log（Session 87/86/85/84/83）

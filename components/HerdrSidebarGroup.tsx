...
export function HerdrSidebarGroup({
  ...
  GroupRow: ComponentType<{ level: number; collapsed: boolean; label: string; title: string; badge: string | null; onToggle: () => void }>;
  ...
  return (
    <section className="mb-3" data-herdr-group>
      <div className="mx-1 flex h-[26px] max-md:h-11 items-center gap-1.5 rounded-md px-1.5 text-ui font-semibold text-ink-strong">
        ...
        <span>Herdr</span>
        {available && <span className="ml-auto text-nano font-normal tabular-nums text-ink-faint">{workspaces.reduce((n, w) => n + w.panes.length, 0)}</span>}
      </div>
      {!available ? (
        <div className="mx-1 min-h-[32px] max-md:min-h-11 rounded-md px-6 py-1.5 text-label text-ink-faint" data-herdr-unavailable title={unavailableText}>{unavailableText}</div>
      ) : workspaces.length === 0 ? (
        <div className="mx-1 min-h-[32px] max-md:min-h-11 px-6 py-1.5 text-label italic text-ink-faint">当前没有 agent pane</div>
      ) : (
        <div>
          ...
          {tree.ungrouped.map(workspace => <div key={workspace.id} data-herdr-workspace={workspace.id}>
            <div className="flex h-[26px] max-md:h-11 items-center px-3 text-label font-medium text-ink-muted"><span className="truncate" title={workspace.label}>{workspace.label}</span></div>
            {renderPanes(workspace.panes)}
          </div>)}
        </div>
      )}
    </section>

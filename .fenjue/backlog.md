...
- **codex-interrupt-leaves-exec-alive** · P1 · **已上线（2026-09-16 15:16，sm-toolkit PR #19 → daemon pid 33157）**：sm-toolkit `fix/codex-interrupt-exec` cc8a90f（interrupt 前快照后代收割 + detached 进程组 close/退出连坐；Sonnet 复核 pass，同源），待合 main + 重启常驻 daemon；真机脚本 `.fenjue/briefs/cxint-harness/run.sh` 待网关 codex 恢复后补跑 · 真实 Codex 线程 `turn/interrupt` 有 request+ack、turn 终态 interrupted，但正在跑的 commandExecution 没有 item/completed，且命令子进程（unifiedExec 会话里的 `sleep 600`，PID 11163）10 秒后仍活。证据：fj-ingress-fresh-start-01bb 迟到信件（`.fenjue/archive/fj-ingress-fresh-start-01bb/mail-late-resend.ndjson` 末条）与该 worktree `~/.herdr/worktrees/sm-toolkit/feat-ingress-fresh-start` 的 pty。影响：Trellis/TUI 里「中断」一个 Codex 会话后命令可能继续跑。查法：codex app-server 的 unified exec 会话在 turn/interrupt 后是否按设计保留；若是，daemon 的 codex 引擎需在 interrupt 时显式结束该 turn 的 exec 会话（或杀进程组）并补发 item/completed(failed)。
...
- **claude-tui-via-mod-api** · P3 · parked（用户 2026-09-16 裁决不做，只留档；**不是** `- [ ]` 队列项，fj 不要自动取）· 「让 Claude Code 官方 TUI 显示 agent-server 线程」的第三条正路：走 `anthropics/claude-code` 公开的插件 API（`mods/types/claude-code.d.ts`，三个内置 mod 源码可参照），不碰泄露源码。可用 hook：`prompt.submit`（可 `{ drop }` 截流转发 AS）、`model.complete`（插件可 `{ value }` 代答 = 引擎替换点）、`ui.render` + Box/Text（transcript 旁 dock pane，放 AS 工具时间线与审批卡）、`ask`、`command.register`、`session.attach`；缺口 `session.send` 标 reserved。若重启：只读 spike 半到一坐席日，hello mod 用 `claude --plugin-dir` 在 2.1.258 / 2.1.266 上验四件事（截 prompt、代答后原生渲染、画 pane、弹审批）。背景与判据见 progress/facts.md 2026-09-15「anthropics/claude-code 公开仓库…」条与 S172 补记 2；direct-connect 与 attach 两线证伪见 decisions 09-15。
